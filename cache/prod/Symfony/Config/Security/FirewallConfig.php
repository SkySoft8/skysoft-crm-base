...
require_once __DIR__.\DIRECTORY_SEPARATOR.'FirewallConfig'.\DIRECTORY_SEPARATOR.'SamlConfig.php';
...
class FirewallConfig 
{
    ...
    public function saml(array $value = []): \Symfony\Config\Security\FirewallConfig\SamlConfig
    {
        if (null === $this->saml) {
            $this->_usedProperties['saml'] = true;
            $this->saml = new \Symfony\Config\Security\FirewallConfig\SamlConfig($value);
        } elseif (0 < \func_num_args()) {
            throw new InvalidConfigurationException('The node created by "saml()" has already been initialized. You cannot pass values the second time you call saml().');
        }
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('saml', $value)) {
            $this->_usedProperties['saml'] = true;
            $this->saml = new \Symfony\Config\Security\FirewallConfig\SamlConfig($value['saml']);
            unset($value['saml']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['saml'])) {
            $output['saml'] = $this->saml->toArray();
        }
